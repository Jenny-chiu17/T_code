<template>
  <div class="border border-gray-200 rounded-lg p-5 hover:border-primary-300 transition-colors">
    ...
    <div class="flex items-start justify-between mb-3">
      ...
      <div v-if="change.similarity !== undefined" class="w-32">
        <SimilarityBar :value="change.similarity" />
      </div>
    </div>

    <!-- 關鍵字匹配分析 -->
    <KeywordAnalysis
      v-if="change.keywordMatches || change.matchSummary"
      :keyword-matches="change.keywordMatches"
      :similarity="change.similarity"
      :match-summary="change.matchSummary"
    />
